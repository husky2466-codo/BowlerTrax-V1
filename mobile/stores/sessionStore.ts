...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  getSessionStats: () => {
    ...
    const strikes = currentShots.filter((s) => s.actualResult === 'strike').length;
    const spares = currentShots.filter((s) => s.actualResult === 'spare').length;
    const opens = currentShots.filter((s) => s.actualResult === 'open').length;

    const speeds = currentShots.filter((s) => s.speedMph).map((s) => s.speedMph!);
    const revRates = currentShots.filter((s) => s.revRateRpm).map((s) => s.revRateRpm!);
    const entryAngles = currentShots.filter((s) => s.entryAngleDeg).map((s) => s.entryAngleDeg!);
